...
# def process_message(user_id, user_name, message_text):
    ...
    if event.source.type == 'group':
        ...
        print("session_query: ", session_query)
        ...
        if reply_text:
            ...
            reply_text_to_insert = ""
            if isinstance(reply_text, list):
                if len(reply_text) > 1:
                    reply_text_to_insert = reply_text[1]
                elif len(reply_text) == 1:
                    reply_text_to_insert = reply_text[0]
                else:
                    print("錯誤：異常空訊息")
            else:
                reply_text_to_insert = reply_text
            try:
                ai_message_response = await supabase_async_client.table("messages").insert({
                    "messageid": str(uuid.uuid4()),
                    "sessionid": new_session_id,
                    "userid": user_id,
                    "content": reply_text_to_insert,
                    "direction": "outbound"
                }, returning="minimal").execute()
...
async def aiPrompt(session_id, user_id, user_name, supabase_async_client, openai_async_client):
    ...
    for tool_call in tool_calls:
        ...
        match tool_call.function.name:
            case "check_scam_phone":
        # if tool_call.function.name == "check_scam_phone":
                function_args = json.loads(tool_call.function.arguments)
                phone_number = function_args.get("phone_number")
                
                # 呼叫工具函式時，將全域的黑名單 set 傳入
            ...
            case "draw_tarot_cards":
                ...
                for message in messages:
                    ...
                    if message['role'] == 'user':
                        message['content'] = tarotToAIsystemPrompt
                messages.append({
                        "tool_call_id": tool_call.id,
                        "role": "tool",
                        "name": tool_call.function.name,
...
# async def scrape_165_async(phone_number: str) -> dict:
#     """
#     非同步爬取 165 網站的查詢結果 (針對新的 SPA 架構)。
#     """
#     # 這是 165 網站背後真正的 API 端點
#     API_URL = "https://165.npa.gov.tw/api/article/A01"
#     headers = {
#         "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36",
#         "Content-Type": "application/json;charset=UTF-8",
#     }
#     # 這是 API 需要的 JSON 格式 payload
#     # "SearchType": 4 對應到您發現的 dataType=4 (電話)
#     payload = {
#         "SearchString": phone_number,
#         "SearchType": 4  
#     }
    
#     try:
#         async with httpx.AsyncClient() as client:
#             # 我們現在是向 API 發送 POST 請求，並攜帶 JSON payload
#             response = await client.post(API_URL, headers=headers, json=payload, timeout=10)
#             response.raise_for_status()

#         data = response.json()
        
#         # 根據 API 回傳的 JSON 結構來判斷結果
#         # 如果 "IsSuccess" 是 true 且 "Data" 列表不是空的，表示有資料
#         if data.get("IsSuccess") and data.get("Data"):
#             details = "此號碼在 165 網站上有被通報的紀錄，風險較高。"
#             # 我們可以從回傳的資料中提取更詳細的資訊
#             first_report = data["Data"][0]
#             report_detail = first_report.get("ReportDetail", "無詳細說明").strip()
#             if report_detail:
#                 details += f"\n摘要：{report_detail}"
...
#     非同步爬取 165 網站的查詢結果。
#     """
#     URL = "https://165.npa.gov.tw/Home/Search"
#     headers = {
#         "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36"
#     }
#     # 165 網站的查詢是透過 POST 表單提交
#     form_data = {"keyword": phone_number}
    
#     try:
#         async with httpx.AsyncClient() as client:
#             response = await client.post(URL, headers=headers, data=form_data, timeout=10)
#             response.raise_for_status()

#         soup = BeautifulSoup(response.text, "lxml")
        
#         # 根據 165 網頁的結構尋找結果
#         # 我們尋找 class="result-box" 的 div 中的 table
#         result_table = soup.find("div", class_="result-box")
#         if result_table and result_table.find("table"):
#             # 如果找到了表格，表示有被通報的紀錄
#             details = "此號碼在 165 網站上有被通報的紀錄，風險較高。"
#             # 您可以進一步解析表格內的文字，但我們先做簡單判斷
#             return {"source": "165全民防騙網", "is_scam": True, "details": details}
#         else:
#             # 如果沒有找到表格，表示尚無通報
#             return {"source": "165全民防騙網", "is_scam": False, "details": "目前尚無詐騙通報紀錄。"}
...
async def perform_scam_phone_check(phone_number_from_ai: str, blacklist: set) -> str:
    ...
    # if not cleaned_number.startswith("09") or len(cleaned_number) != 10:
    #     return json.dumps({"status": "invalid_format", "summary": "這不是一個有效的台灣手機號碼格式。"})
...
def normalize_phone_for_165(phone_number_str: str) -> str:
    ...
    cleaned_number = re.sub(r'\D', '', phone_number_str)
    # \D 在正規表達式中代表 "任何非數字 (non-digit)" 的字元。
    # re.sub(r'\D', '', ...) 的意思就是：找到所有不是數字的字元，並把它們替換成空字串（也就是刪除）。
