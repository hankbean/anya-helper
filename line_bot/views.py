from django.shortcuts import render

from django.http import HttpRequest, HttpResponse, HttpResponseBadRequest
from django.views.decorators.csrf import csrf_exempt
from django.conf import settings
import asyncio

from linebot.v3 import WebhookParser
from linebot.v3.exceptions import InvalidSignatureError
from linebot.v3.messaging import (
    AsyncApiClient,
    Configuration,
    AsyncMessagingApi,
    ReplyMessageRequest,
    TextMessage,
    ShowLoadingAnimationRequest,
)
from linebot.v3.webhooks import MessageEvent, TextMessageContent

from . import handlers
from .services.web_crawler import ptt_crawler
from openai import AsyncOpenAI
from supabase import acreate_client
from imgurpython import ImgurClient

# Create your views here.
configuration = Configuration(access_token=settings.LINE_CHANNEL_ACCESS_TOKEN)
parser = WebhookParser(settings.LINE_CHANNEL_SECRET)

# Django 的非同步視圖
@csrf_exempt
async def callback(request: HttpRequest):
    if request.method == "POST":
        signature = request.headers.get('X-Line-Signature', '')
        body = request.body.decode('utf-8')

        try:
            events = parser.parse(body, signature)
        except InvalidSignatureError:
            print("Invalid signature. Please check your channel access token/channel secret.")
            return HttpResponseBadRequest() # 回傳 400 錯誤

        await handle_events(events)
        return HttpResponse('ok') # 回傳 200 OK
    
    return HttpResponseBadRequest()

async def handle_events(events):
    async with AsyncApiClient(configuration) as async_api_client, \
            AsyncOpenAI(api_key=settings.OPENAI_API_KEY) as openai_client:
        supabase_client = await acreate_client(settings.SUPABASE_URL, settings.SUPABASE_KEY)
        line_bot_api = AsyncMessagingApi(async_api_client)
        imgur_client = ImgurClient(settings.IMGUR_CLIENT_ID, settings.IMGUR_CLIENT_SECRET)

        tasks = []
        for event in events:
            if isinstance(event, MessageEvent) and isinstance(event.message, TextMessageContent):
                tasks.append(process_message_event(event, line_bot_api, supabase_client, openai_client, imgur_client))

        if tasks:
            await asyncio.gather(*tasks)

async def process_message_event(event, line_bot_api, supabase_client, openai_client, imgur_client):
    await line_bot_api.show_loading_animation(#不發送訊息的話如何中斷動畫
        ShowLoadingAnimationRequest(chatId=event.source.user_id, loadingSeconds=60)
    )
    command_parts = event.message.text.split()
    match command_parts:
        # case ["搜尋", keyword]:
        #     await handle_search_ptt(event, keyword)
        # case ["搜尋", *keywords]: # 匹配多個關鍵字
        #     full_keyword = " ".join(keywords)
        #     await handle_search_ptt(event, full_keyword)
        case ["幫助"] | ["help"]:
            await handlers.handle_show_help_message(event, line_bot_api)
        case ["抽正牌"]:
            await handlers.handle_draw_tarot_card(event, line_bot_api, imgur_client)
        case ["骰子卡"]:
            await handlers.handle_roll_astro_dice(event, line_bot_api)
        # case ["進階骰子卡"]:
        #     await handlers.handle_roll_astro_dice_plus(event, line_bot_api)
        case ["六芒星說明"]:
            await handlers.handle_hexagram_explanation(event, line_bot_api)
        case ["表特/"]:
            await handlers.handle_ptt_beauty(event, line_bot_api)
        case ["test"]:
            await handlers.handle_test(event, line_bot_api)
        case _:
            await handlers.handle_default_message(event, line_bot_api, supabase_client, openai_client)